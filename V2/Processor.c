...
char * PSW_BITS_names [INTERRUPTTYPES]={"POWEROFF_BIT","UNUSED","UNUSED",
...
void Processor_FetchInstruction() {
    ...
    if (MMU_readMemory()==MMU_SUCCESS) {
      ...
      ComputerSystem_DebugMessage(HARDWARE,"csdsds",registerIR_CPU.operationCode," ",registerIR_CPU.operand1," ",registerIR_CPU.operand2, " ");
    }
    else
...
void Processor_DecodeAndExecuteInstruction() {
    ...
    ComputerSystem_DebugMessage(HARDWARE,"sRdsRdsds","(PC: ",registerPC_CPU,", Accumulator: ",registerAccumulator_CPU,", PSW: ", registerPSW_CPU, ")\n");
}
