...
void ComputerSystem_PowerOff() {
	...
	ComputerSystem_DebugMessage(HARDWARE, "s", "END of the simulation");
	exit(0);
}
...
void ComputerSystem_PrintProgramList(){
  ...
  ComputerSystem_DebugMessage(INIT, "s", "User program list:\n");
     ...
     int i = 0;
     while (1) {
         upd = userProgramsList[i];
         if (upd == NULL) {
             return;
         }
         ComputerSystem_DebugMessage(INIT, "sCssCds", "\tProgram [", upd->executableName,
                                     "] with arrival time [", upd->arrivalTime, "]\n");
         i++;
     }
